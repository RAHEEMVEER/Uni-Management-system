...
class Course {
  ...
  setInstructor(instructor: Teacher) {
...
console.log(
  chalk.green.bold.italic(
    `\t     WELLCOME TO ${universityName} MANAGEMENET SYSTEM:`
  )
);
console.log(
  chalk.yellow.bold.italic(
...
async function Main() {
    ...
    if (departments.length === 0) {
      ...
      let selectDept = await inquirer.prompt([
        {
          name: "department",
          type: "list",
          choices: departments.map((transform) =>
            chalk.green.bold.italic(transform.name.toUpperCase())
          ),
      ...
      let course = await inquirer.prompt([
        ...
        },
        {
          name: "course",
          type: "input",
          message: chalk.grey.bold.italic(
            "ENTER COURSE NAME YOU WANT TO ADD: "
          ),
        },
      ]);
      ...
      let selectedDepartment = departments.find(
        (dept) => chalk.green.bold.italic(dept.name) === selectDept.department
      );
      ...
      let remStudent = await inquirer.prompt([
        {
          name: "removeStudent",
          type: "list",
          choices: students.map((remStudent) =>
            chalk.redBright.bold.italic(remStudent.getName())
          ),
          message: chalk.magentaBright.bold.italic(
            "SELECT STUDENT YOU WANT TO REMOVE FROM UNIVERSITY."
          ),
        },
      ]);
      let studentIndex = students.findIndex(
        (student) =>
          chalk.redBright.bold.italic(student.getName()) ===
      ...
      if (studentIndex !== -1) {
        ...
        console.log(
          chalk.redBright.bold.italic(`\n\t STUDENT `) +
            ...
            chalk.redBright.bold.italic(
              ` SUCCESSFULLY REMOVE FROM UNIVERSITY.\n`
            )
        );
      }
    }
      ...
      let remTeacher = await inquirer.prompt([
        {
          name: "removeTeacher",
          type: "list",
          choices: teachers.map((remTeacher) =>
            chalk.redBright.bold.italic(remTeacher.getName())
          ),
          message: chalk.magentaBright.bold.italic(
            "SELECT TEACHER YOU WANT TO REMOVE FROM UNIVERSITY."
          ),
        },
      ]);
      let teacherIndex = teachers.findIndex(
        (teacherIndex) =>
          chalk.redBright.bold.italic(teacherIndex.getName()) ===
  ...
  async function enrollStudentCourse() {
    if (students.length === 0) {
      ...
      let courseNames = courses.map((course) =>
        chalk.yellow.bold.italic(course.name)
      );
      ...
      let studentChoice = await inquirer.prompt([
        {
          ...
          message: chalk.yellow.bold.italic(
            "SELECT STUDENT TO ENROLL FOR COURSE."
          ),
        },
      ]);
      ...
      let courseChoice = await inquirer.prompt([
        {
          ...
          message: chalk.green.bold.italic(
            "SELECT COURSE TO ENROLL STUDENT FOR."
          ),
        },
      ]);
      let selectedCourse = courses.find(
        (course) =>
          chalk.yellow.bold.italic(course.name) === courseChoice.enrollCourse
      );
  ...
  async function assingCourseTeacher() {
    if (teachers.length === 0) {
      ...
      let teacherNames = teachers.map((teacher) =>
        chalk.magentaBright.bold.italic(teacher.getName())
      );
      let courseNames = courses.map((course) =>
        chalk.cyanBright.bold.italic(course.name)
      );
      let teacherChoice = await inquirer.prompt([
        {
          ...
          message: chalk.cyan.bold.italic(
            "SELECT TEACHER TO ASSIGN COURSE TO."
          ),
        },
      ]);
      ...
      let selectedCourse = courses.find(
        (coursee) =>
          chalk.cyanBright.bold.italic(coursee.name) ===
  ...
  while (iscontinue) {
    ...
    if (action.action === chalk.red.bold.italic("EXIT FROM MANAGEMENT:")) {
      ...
      setTimeout(() => {
        console.log(
          chalk.yellow.bold.italic(
            "\t  THANKS FOR VISIT KARACHI UNIVERSITY MANAGEMENT."
          )
        );
        const authorName = "RAHEEM VEER";
        console.log(
          chalk.cyan.bold.italic(`\t  THIS MANAGEMENT CREATED BY `) +
